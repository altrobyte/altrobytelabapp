...
class ApiService {
  ...
  static Future<List<dynamic>> getDemos() async {
    final res = await safeGet(Uri.parse('${ApiConstants.baseUrl}/demos'),
        headers: _headers(null));
    return (_parse(res)['demos'] as List?) ?? [];
  }

  /// Takes a seat. A 409 means it filled up while the sheet was open.
  static Future<Map<String, dynamic>> registerForDemo({
    required int demoId,
    required String name,
    required String phone,
    String email = '',
    ...
    final res = await safePost(
      Uri.parse('${ApiConstants.baseUrl}/demos/$demoId/register'),
      headers: _headers(null),
      body: jsonEncode({
        'name': name,
        'phone': phone,
  ...
  // ── Demo admin ─────────────────────────────────────────────────────────
  static Future<List<dynamic>> adminGetDemos() async {
    final prefs = await SharedPreferences.getInstance();
    final res = await safeGet(Uri.parse('${ApiConstants.baseUrl}/demos/admin/list'),
        headers: _headers(prefs.getString('token')));
    return (_parse(res)['demos'] as List?) ?? [];
  }

  /// Creates when [id] is null, updates otherwise.
  static Future<void> adminSaveDemo(Map<String, dynamic> body, {int? id}) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token');
    final uri = Uri.parse(id == null
        ? '${ApiConstants.baseUrl}/demos/admin'
        : '${ApiConstants.baseUrl}/demos/admin/$id');
    final res = id == null
  ...
  static Future<void> adminDeleteDemo(int id) async {
    final prefs = await SharedPreferences.getInstance();
    final res = await http.delete(
        Uri.parse('${ApiConstants.baseUrl}/demos/admin/$id'),
        headers: _headers(prefs.getString('token')));
    _parse(res);
  }
  ...
  static Future<List<dynamic>> adminGetDemoAttendees(int id) async {
    final prefs = await SharedPreferences.getInstance();
    final res = await safeGet(
        Uri.parse('${ApiConstants.baseUrl}/demos/admin/$id/attendees'),
        headers: _headers(prefs.getString('token')));
